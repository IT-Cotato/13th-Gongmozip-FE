...
// TODO(backend): 사용자 프로필/통계/협업거리 API 연동 전까지 목데이터로 임시 표시
const MOCK_USER = {
  name: "김철수",
  testStatus: "검사 전",
};
...
const MOCK_COLLABORATIVE_DISTANCE = 100;
...
const MENU_SECTIONS: MenuSection[] = [
  {
    title: "정보관리",
    items: [{ label: "회원정보 수정" }, { label: "비밀번호 변경" }],
  },
  {
    title: "고객지원",
  ...
  {
    title: "법적정보 및 기타",
    items: [{ label: "공모집 서비스 이용약관" }, { label: "개인정보 처리방침" }],
  },
  {
    items: [{ label: "로그아웃" }],
  },
        ...
        <section className="flex flex-col items-center">
          <div className="flex w-full items-start gap-4 px-6 py-4">
            ...
            <div className="flex flex-1 flex-col items-start gap-2">
              <span className="rounded-full bg-[#C8C8C8] px-2 py-1 text-xs font-semibold text-white">
                {MOCK_USER.testStatus}
              </span>
          ...
          <div className="flex w-full flex-col items-start px-4">
            ...
            >
              <p className="text-[13px] leading-[1.25] font-semibold text-[#616161]">협업거리</p>
              <CollaborativeDistance currentMeters={MOCK_COLLABORATIVE_DISTANCE} />
            </div>
          </div>
        </section>
