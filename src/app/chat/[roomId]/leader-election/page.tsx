...
type SheetState = "closed" | "willingness" | "candidateVote" | "complete" | "result";
...
export default function LeaderElectionPage() {
  ...
  const [isWillingnessSubmitted, setIsWillingnessSubmitted] = useState(false);
  ...
  const submitWillingness = () => {
    const firstCandidate = safeCandidates[0] ?? fallbackCandidate;

    setSelectedCandidateId(firstCandidate.id);
    setIsWillingnessSubmitted(true);
  ...
  return (
    <main className="relative flex h-full w-full flex-col overflow-hidden bg-white pt-[env(safe-area-inset-top)] text-color-gray-850">
      ...
      >
        ...
        {isWillingnessSubmitted ? (
          <BotMessage
            body="바로 팀장 선출 투표를 하도록 하겠습니다. 팀장 지원자 분들은 되도록이면 프로필을 공개로 돌려, 팀원들이 볼 수 있도록 해주세요."
            buttonLabel="팀장 투표하기"
        ...
        <div className="absolute inset-0 z-40 flex items-end bg-color-gray-850/60">
          ...
          {sheetState === "candidateVote" ? (
            <LeaderCandidateVoteSheet
              ...
              onSubmit={() => setSheetState("complete")}
            />
          ) : null}

          {sheetState === "complete" ? (
            <VoteCompleteSheet onShowResult={() => setSheetState("result")} />
          ) : null}

          {sheetState === "result" ? (
            <VoteResultSheet leader={selectedCandidate} onClose={() => setSheetState("closed")} />
          ) : null}
        </div>
      ) : null}
    </main>
  ...
  return (
    <article className="flex w-full items-start gap-2">
      ...
      <div className="flex min-w-0 flex-1 flex-col items-start gap-1">
        ...
        <button
          className="mt-1 flex h-9 w-[230px] items-center justify-center rounded-[10px] bg-color-coral-500 px-2 text-[13px] leading-[1.25] font-semibold text-white"
...
function VoteResultSheet({
  ...
  return (
    <BottomSheet className="min-h-[402px]">
      <div className="flex flex-col items-center px-6 pb-4">
        <div className="w-full">
          ...
          <p className="mt-3 px-2 text-[13px] leading-[1.5] text-color-gray-850">
            <span className="font-semibold text-color-coral-700">{leader.name} 님</span>이
            팀장으로 확정되었습니다🎉
          </p>
        </div>
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          ...
          <span className="flex h-8 items-center justify-center rounded-full bg-color-coral-500 px-3 text-[15px] leading-[1.25] font-semibold text-white">
            {leader.name}
          </span>
        </div>
      </div>

      <SheetButton label="완료" onClick={onClose} tone="neutral" />
    </BottomSheet>
  );
}
